...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-30 bg-black transition-opacity duration-300 ${isOpen ? 'opacity-50' : 'opacity-0 pointer-events-none'}`}
        onClick={toggleSidebar}
      ></div>

      <div
        ref={sidebarRef}
        className={`fixed top-0 left-0 z-40 w-[355px] h-screen p-4 overflow-y-auto bg-black text-white custom-scrollbar transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <button onClick={toggleSidebar} className="absolute top-0 right-0 bg-[#444444] p-1">
          <AiOutlineClose />
        </button>
        <img
        ...
        <div className="border-4 border-[#444444] h-[27.5rem] rounded p-1">
          <div className="text-center mb-8 border-4 border-[#444444] p-12 rounded">
            ...
            <div className="mt-4 text-sm text-[#8F8F8F]">
              ...
              <label htmlFor="terms">By signing up, you agree to the our terms and our Privacy Policy agreement.</label>
            </div>
          </div>
        </div>
